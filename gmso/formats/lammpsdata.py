...
def _write_header(out_file, top, atom_style, dihedral_parser):
    ...
    # TODO: atomtypes should include virtual sites
...
def _write_site_data(out_file, top, atom_style, base_unyts, cfactorsDict):
    ...
    if atom_style == "atomic":
        ...
        atom_line = "{index:d}\t{type_index:d}\t{x:.8}\t{y:.8}\t{z:.8}\t{moleculeid:d}\t{charge:.8}\t{mux:.8}\t{muy:.8}\t{muz:.8}\t{radius:.8}\t{rmass:.8}\n"
    ...
    for i, site in enumerate(itertools.chain(top._sites, top._virtual_sites)):
